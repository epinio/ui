...
const zipParts = this.resources[0].applicationParts.filter(
...
progressBar = computed(() => {
  get() {
    return Object.keys(this.percentages).reduce(
      (acc, part) => acc + (this.percentages[part] * (partsWeight[part] || 1)), 0);
  },
  set(value) {
