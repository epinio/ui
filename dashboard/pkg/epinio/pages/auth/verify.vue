...
export default {
  ...
  async fetch({ store, route, redirect }) {
    ...
    store.dispatch('epinio/initialiseOidcClient');
    const oidcUserManager = store.getters['epinio/oidcClient']();

    debugger;
    let user;

    try {
      user = await oidcUserManager.signinRedirectCallback(route.url);
    } catch (e) {
      console.error('token warning', e);
    }
    console.warn('user', user);
    debugger;

    // TODO: RC wire in oidcUserManager.getUser() and it's auth to requests to epinio
    // TODO: RC error handling. no state / code. and above. and below
    // TODO: RC clean up whole file
